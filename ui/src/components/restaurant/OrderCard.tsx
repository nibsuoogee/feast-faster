...
// Component helpers
const ChargingStatus = ({ chargePercentage = 0 }: { chargePercentage?: number }) => (
    ...
    <div className="relative z-10 flex items-start gap-3">
      ...
      <div className="flex-1">
        ...
        <div className="text-sm text-gray-700 font-medium">
          Current charge: <span className="text-blue-700 font-semibold">{chargePercentage}%</span>
        </div>
      </div>
    </div>
...
const ETACountdown = ({ etaText, etaTime }: { etaText: string; etaTime: string }) => (
    ...
    <Card className="p-5 rounded-xl border border-gray-200 shadow-sm hover:shadow-md transition-all duration-200 hover:border-gray-300 bg-white">
      <div className="space-y-3">
        ...
        {order.status === 'picked_up' ? (
          ...
          <ETACountdown etaText={etaText} etaTime={formatTime(order.customerETA)} />
        )}
